...
#include "Systick.h"
static u8  fac_us=0;							//用于实现us延时倍乘数的全局变量			   
static u16 fac_ms=0;							//用于实现ms延时倍乘数
		   
/*初始化延迟函数
SYSTICK的时钟固定为AHB时钟(168MHZ频率)的1/8，SYSCLK:系统时钟频率  */
void Systick_init(u8 SYSCLK)
{
 	SysTick_CLKSourceConfig(SysTick_CLKSource_HCLK_Div8); 
	fac_us=SYSCLK/8;						//8分频后，fac_us就是21
...
/* void delay_ms(u16 nms)
{	 	 
	u8 repeat=nms/540;						//这里用540,是考虑到可能超频使用,
